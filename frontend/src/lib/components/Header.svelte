	...
	<div class="flex gap-x-4">
		...
		<div class="2xl:hidden relative group">
			{#if sidebarVisible == false}
				...
				<div class="absolute w-max top-0 left-10 mt-2 ml-2 p-2 bg-gray-800 text-white rounded opacity-0 invisible group-hover:opacity-100 group-hover:visible group-hover:translate-x-0">
					{$LL.ShowMenu()}
				</div>
			{/if}
		</div>
